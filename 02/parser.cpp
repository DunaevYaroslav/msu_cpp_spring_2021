...
const std::string TokenParser::Parse(const std::string &text)
{
    ...
    if (start_func_ != nullptr)
    {
        ...
        while (!iss.eof())
        {
            ...
            if (isnumber && ((token.length() < std::string("18446744073709551616").length()) ||
                             ((token.length() == std::string("18446744073709551616").length()) && (token < std::string("18446744073709551616")))))
            {
                if (digit_func_ == nullptr)
                {
                    throw std::runtime_error("Функция для обработки чисел не была инициализирована");
                }
                uint64_t t = std::stoull(token);
                digit_func_(t);
                oss << t << " ";
            }
            else
